...
namespace RQ.RevitUtils.ExtensibleStorageUtils.SchemaWrapperTools
{
    ...
    public class SchemaWrapper
    {
...
#if (REVIT2016 || REVIT2017 || REVIT2018 || REVIT2019 || REVIT2020 || REVIT2021)
                genericAddFieldMethodInstantiated.Invoke(swReturn, new object[] { currentField.FieldName, GetUnitType(currentField), swSub });
#else
...
#else
                if (currentFieldData.Unit != ExtendStorageManage.GetUndefinedUnitType())
        ...
        public override string ToString()
        {
            ...
            foreach (FieldData fd in Data.DataList)
            {
        ...
        private void DumpAllSchemaEntityData<EntityType>(EntityType storageEntity, Schema schema, StringBuilder strBuilder)
        {
            ...
            foreach (Field currentField in schema.ListFields())
            {
                ...
                //Get the method.
                MethodInfo getFieldDataAsStringMethod = typeof(SchemaWrapper).GetMethod("GetFieldDataAsString", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.InvokeMethod, Type.DefaultBinder, new Type[] { typeof(Field), typeof(Entity), typeof(StringBuilder) }, pmodifiers);
        ...
        private void GetFieldDataAsString<KeyType, FieldType>(Field field, Entity entity, StringBuilder strBuilder)
        {
...
#if (REVIT2016 || REVIT2017 || REVIT2018 || REVIT2019 || REVIT2020 || REVIT2021)
            UnitType fieldUnit = GetUnitType(field);
#else
            ...
            if (fieldUnit == ExtendStorageManage.GetUndefinedUnitType())
            {
...
#else
                methodOverloadSelectionParams = new Type[] { typeof(Field), typeof(UnitTypeId) };
            ...
            if (field.ContainerType == ContainerType.Simple)
            {
                ...
                else
                {
                    ...
                    strBuilder.AppendLine("Field: " + field.FieldName + ", Type: " + field.ValueType.ToString() + ", Value: " + retval + ", Unit: " + GetUnitType(field).ToString() + ", ContainerType: " + field.ContainerType.ToString());
                }
            }
            else if (field.ContainerType == ContainerType.Array)
            {
...
#if (REVIT2016 || REVIT2017 || REVIT2018 || REVIT2019 || REVIT2020 || REVIT2021)
        public static UnitType GetUnitType(Field field)
        {
            return GetUnitType(field);        
        }
#else
        public static ForgeTypeId GetUnitType(Field field)
        {
            return field.GetSpecTypeId();
        }
#endif
